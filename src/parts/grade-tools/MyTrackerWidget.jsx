...
const normalizeSemesterMeta = (currentSemester, totalSemesters) => {
  const safeCurrent =
    Number.isInteger(currentSemester) && currentSemester > 0
      ...
      : 1;
  let safeTotal =
    Number.isInteger(totalSemesters) && totalSemesters > 0 ? totalSemesters : 8;
...
const getUserCgpaStateKey = (activeUserId) =>
...
const getInitialState = (storageKey) => {
  try {
    ...
    return {
      ...
      remainingUnits:
        typeof parsed.remainingUnits === "string" ? parsed.remainingUnits : "",
      ...
      classification:
        typeof parsed.classification === "string"
          ...
          : null,
      showDashboardCard:
        typeof parsed.showDashboardCard === "boolean"
          ? parsed.showDashboardCard
          : true,
      showDashboardClassification:
        typeof parsed.showDashboardClassification === "boolean"
          ? parsed.showDashboardClassification
          : true,
      onboardingCompleted: Boolean(parsed.onboardingCompleted),
      currentSemester: safeCurrent,
      totalSemesters: safeTotal,
      previousSemesters: Array.isArray(parsed.previousSemesters)
        ? parsed.previousSemesters
        : [],
    };
  } catch (error) {
    return {
...
function MyTrackerWidget({ activeUserId = "guest" }) {
  const userCgpaStateKey = useMemo(
    () => getUserCgpaStateKey(activeUserId),
    [activeUserId],
  );
  const initialState = useMemo(
    () => getInitialState(userCgpaStateKey),
    [userCgpaStateKey],
  );
  ...
  const [showDashboardCard, setShowDashboardCard] = useState(
    initialState.showDashboardCard,
  );
  const [showDashboardClassification, setShowDashboardClassification] =
    useState(initialState.showDashboardClassification);
  const [onboardingCompleted, setOnboardingCompleted] = useState(
    initialState.onboardingCompleted,
  );
  const [currentSemester, setCurrentSemester] = useState(
    initialState.currentSemester,
  );
  const [totalSemesters, setTotalSemesters] = useState(
    initialState.totalSemesters,
  );
  const [previousSemesters, setPreviousSemesters] = useState(
    initialState.previousSemesters,
  );
  ...
  const applyCgpaState = (state) => {
    ...
    setRemainingUnits(
      typeof state.remainingUnits === "string" ? state.remainingUnits : "",
    );
    setCgpa(Number.isFinite(state.cgpa) ? state.cgpa : null);
    setClassification(
      typeof state.classification === "string" ? state.classification : null,
    );
    setShowDashboardCard(
      typeof state.showDashboardCard === "boolean"
        ? state.showDashboardCard
        : true,
    );
    ...
    setPreviousSemesters(
      Array.isArray(state.previousSemesters) ? state.previousSemesters : [],
    );
  };
  ...
  const loadAcademicEvents = async () => {
    ...
    try {
      ...
      setAcademicNotice(
        error.message || "Unable to load academic reminders right now.",
      );
    }
  };
      ...
      .map((point, index) => {
        const x =
          semesterHistory.length === 1
            ? 0
            : (index / (semesterHistory.length - 1)) * 100;
  ...
  const goalProjection = useMemo(() => {
    ...
    const requiredTotalWeighted =
      boundedTarget * (stats.totalUnits + plannedUnits);
  ...
  const examNeedNotifications = useMemo(() => {
    ...
    return courses
      ...
      .sort((left, right) => {
        ...
        const rightWeight =
          right.type === "risk" ? 0 : right.type === "need" ? 1 : 2;
  ...
  const semesterProjection = useMemo(() => {
    ...
    const estimatedSemesters = Math.min(
      8,
      Math.max(1, Math.ceil(goalProjection.plannedUnits / 18)),
    );
    const points = [];

    for (let semester = 1; semester <= estimatedSemesters; semester += 1) {
      const completedFutureUnits =
        (goalProjection.plannedUnits * semester) / estimatedSemesters;
      const projectedWeighted =
        stats.totalWeighted +
        goalProjection.requiredFutureGpa * completedFutureUnits;
      const projectedUnits = stats.totalUnits + completedFutureUnits;
      const projectedCgpa =
        projectedUnits > 0 ? projectedWeighted / projectedUnits : 0;
  ...
  const chartPath = useMemo(() => {
    ...
    return semesterProjection
      .map((point, index) => {
        const x =
          semesterProjection.length === 1
            ? 0
            : (index / (semesterProjection.length - 1)) * 100;
        const y =
          100 - (Math.min(5, Math.max(0, point.projectedCgpa)) / 5) * 100;
        return `${x},${y}`;
      })
      .join(" ");
  }, [semesterProjection]);
  ...
  const handleAcknowledgeAcademicEvent = async (eventId) => {
    ...
    try {
      ...
      setAcademicNotice(
        error.message || "Unable to update the reminder right now.",
      );
    }
  };
  ...
  const handleUpdateCourse = (id, key, value) => {
    ...
    setCourses((prev) =>
      prev.map((course) =>
        course.id === id ? { ...course, [key]: nextValue } : course,
      ),
    );
  };
  ...
  const handleAdvanceSemester = () => {
    ...
    if (!Number.isFinite(currentCgpa)) {
      setSemesterTransitionNotice(
        "Enter course scores and calculate CGPA before ending the semester.",
      );
      return;
    }
    ...
    setPreviousSemesters((prev) => {
      const withoutCurrent = prev.filter(
        (entry) => Number(entry?.semester) !== currentSemester,
      );
      return [
        ...withoutCurrent,
        {
    ...
    setClassification(null);
    setSemesterTransitionNotice(
      `Semester ${currentSemester + 1} started. Add new courses for this semester.`,
    );
  };
    ...
    <div className={styles.widget}>
      ...
      <div className={styles.preferenceRow}>
        <label className={styles.preferenceToggle}>
          <input
            ...
            onChange={(event) =>
              handleDashboardCardToggle(event.target.checked)
            }
          />
        ...
        <label className={styles.preferenceToggle}>
          <input
            ...
            onChange={(event) =>
              handleDashboardClassToggle(event.target.checked)
            }
          />
      ...
      <div className={styles.semesterInfo}>
        <p className={styles.semesterText}>
          Semester <strong>{currentSemester}</strong> of{" "}
        ...
        <div className={styles.semesterActions}>
          ...
          >
            End Semester {currentSemester} and Start Semester{" "}
            {Math.min(totalSemesters, currentSemester + 1)}
          </button>
        </div>
        {semesterTransitionNotice && (
          <p className={styles.semesterNotice}>{semesterTransitionNotice}</p>
        )}
      </div>
      ...
      <section className={styles.historySection}>
        ...
        <p className={styles.historyHint}>
          Track how your CGPA moves from semester to semester.
        </p>
        ...
        <div className={styles.historyHighlights}>
          ...
          <div className={styles.historyHighlightCard}>
            ...
            <p
              className={`${styles.historyHighlightValue} ${styles.historyHighlightToneRisk}`}
            >
        ...
        {semesterHistory.length < 2 ? (
          <p className={styles.historyEmpty}>
            Add previous semester CGPAs (and calculate current CGPA) to see
            movement.
          </p>
        ) : (
          <>
      ...
      <section className={styles.reminderSection}>
        ...
        <p className={styles.reminderHint}>
          Tests and exams added in the To-Do Planner appear here after their
          reminder time, so you can enter the score into the tracker.
        </p>
        {academicNotice && (
          <p className={styles.reminderNotice}>{academicNotice}</p>
        )}
        {academicReminders.length === 0 ? (
          <p className={styles.reminderEmpty}>
            No score reminders are due right now.
          </p>
        ) : (
          <div className={styles.reminderList}>
            {academicReminders.map((event) => (
              <article key={event.id} className={styles.reminderCard}>
                <div>
                  <p className={styles.reminderBadge}>
                    {event.taskType.replace("-", " ")}
                  </p>
      ...
      <section className={styles.examAlertSection}>
        ...
        <p className={styles.examAlertHint}>
          Based on your CA/Test entries
          {goalProjection ? " and current CGPA target context" : ""}, here is
          what you need in each exam.
        </p>
        {examNeedNotifications.length === 0 ? (
          <p className={styles.examAlertEmpty}>
            Enter Test 1 and Test 2 scores to get exam-needed alerts.
          </p>
        ) : (
      ...
      <section className={styles.goalSection}>
        ...
        <p className={styles.goalHint}>
          Set a target and estimate how many credit units you still have left.
        </p>
          ...
          <div
            className={`${styles.projectionCard} ${
              goalProjection.status === "unachievable"
                ? styles.projectionRisk
                : styles.projectionGood
            }`}
          >
            <p className={styles.projectionLine}>
              Required average future GPA:{" "}
              <strong>
                {Math.max(0, goalProjection.requiredFutureGpa).toFixed(2)}
              </strong>
            </p>
            <p className={styles.projectionLine}>
              Required weighted points in remaining units:{" "}
              <strong>
                {Math.max(0, goalProjection.requiredFutureWeighted).toFixed(2)}
              </strong>
            </p>
            ...
            {goalProjection.status === "already-met" && (
              <p className={styles.projectionStatus}>
                Great news: your current CGPA trend already meets this target.
              </p>
            )}
            {goalProjection.status === "unachievable" && (
              <p className={styles.projectionStatus}>
                This target is not reachable with the entered remaining units
                under a 5.00 grading cap.
              </p>
            )}
          </div>
        )}
        ...
        {goalProjection && semesterProjection.length > 0 && (
          <div className={styles.chartSection}>
            ...
            <div className={styles.chartLegend}>
              ...
              <span>
                Projected CGPA path (up to {semesterProjection.length}{" "}
                semesters)
              </span>
            </div>
            ...
            {currentCgpa !== null && (
              <p className={styles.currentInfo}>
                Current CGPA baseline: {currentCgpa.toFixed(2)}
              </p>
            )}
          </div>
        )}
