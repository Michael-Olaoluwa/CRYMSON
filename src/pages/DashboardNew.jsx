      ...
      <main className={styles.main}>
              ...
              <div className={styles.miniChart}>
                {monthLabels.map((l, i) => (
                  <div key={l} className={styles.miniBarCol}>
                    <div className={styles.miniBar} style={{ height: `${Math.min(100, (monthTotals[i] / Math.max(...monthTotals, 1)) * 100)}%`, background: '#8b5cf6' }} />
        ...
        <div className={styles.quickActions}>
          ...
          <div className={styles.actionsRow}>
            <button className={styles.actionBtn} style={{ '--accent': '#2dd4a7' }} onClick={onNavigateToTime}>
              <span className={styles.actionIcon}>▶</span>
              <span className={styles.actionLabel}>Start Study</span>
            </button>
            <button className={styles.actionBtn} style={{ '--accent': '#8b5cf6' }} onClick={onNavigateToTodo}>
              <span className={styles.actionIcon}>+</span>
              <span className={styles.actionLabel}>New Task</span>
            </button>
            <button className={styles.actionBtn} style={{ '--accent': '#2dd4a7' }} onClick={onNavigateToUserCGPA}>
              <span className={styles.actionIcon}>◎</span>
              <span className={styles.actionLabel}>Calculate CGPA</span>
            </button>
            <button className={styles.actionBtn} style={{ '--accent': '#8b5cf6' }} onClick={onNavigateToFinance}>
              <span className={styles.actionIcon}>¤</span>
                ...
                <div className={styles.chartBars}>
                  {coursePerformanceData.map((c, i) => (
                    <div key={i} className={styles.chartBarCol}>
                      <div className={styles.chartBar} style={{ height: `${Math.min(100, (c.score / 100) * 100)}%`, background: '#2dd4a7', borderRadius: '3px 3px 0 0' }} />
            ...
            <div className={styles.chartArea}>
              <div className={styles.chartBars}>
                {weeklyChartData.map((d, i) => (
                  <div key={i} className={styles.chartBarCol}>
                    <div className={styles.chartBar} style={{ height: `${Math.min(100, (d.minutes / Math.max(...weeklyChartData.map(x => x.minutes), 1)) * 70)}%`, background: '#8b5cf6', borderRadius: '3px 3px 0 0' }} />
